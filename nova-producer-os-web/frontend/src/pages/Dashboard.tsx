...
export default function Dashboard({ onBack }: Props) {
  ...
  const handleDragOver = (e: React.DragEvent, stage: ProducerStage) => {
    e.preventDefault();
    if (stage === "slate" || stage === "package" || stage === "finance" || stage === "launch") {
  ...
  const handleDrop = (e: React.DragEvent, stage: ProducerStage) => {
    ...
    if (file && (stage === "slate" || stage === "package" || stage === "finance" || stage === "launch")) {
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="space-y-5 animate-fadeIn" style={{ animationDelay: "150ms" }}>
          <div className="flex items-center justify-between">
            ...
            <p className="text-sm text-slate-500">Drag a file onto any ready stage</p>
          </div>
          <div className="grid gap-4">
            {stages.map((stage, idx) => {
              const needsParam = stage === "slate" || stage === "package" || stage === "finance" || stage === "launch";
              const paramLabel =
                stage === "slate" ? "Script file" :
                stage === "package" ? "Script or PDF" :
                stage === "finance" ? "Project name" :
                "Project name";
...
## Logline
A one-sentence hook that captures the essence of your story.

## Genre
Psychological thriller / Drama / Sci-Fi

## Synopsis
Write a short paragraph summarizing the plot, central conflict, and emotional stakes.

## Characters
- **Protagonist**: Name, age, occupation, core desire, flaw.
- **Antagonist**: Name, motivation, what they represent.
- **Supporting**: Key allies or obstacles.

## Themes
What universal ideas does your story explore?

## Tone & Visual Style
Describe the mood, color palette, and cinematic references.
`;
                const blob = new Blob([content], { type: "text/markdown" });
                const url = URL.createObjectURL(blob);
                const a = document.createElement("a");
                a.href = url;
                a.download = "sample-script.md";
                a.click();
                      ...
                      <div className="pl-4">
                        <div
                          className={`drag-zone flex items-center gap-3 rounded-xl border px-4 py-3 bg-slate-900/40 ${
                            isDragOver ? "border-cyan-500/50 bg-cyan-500/5" : "border-slate-800/60"
                          }`}
                        >
                          ...
                          <input
                            ...
                            onChange={(e) => setStageParams((prev) => ({ ...prev, [stage]: e.target.value }))}
                          ...
                          {stage === "slate" && (
                            <button
                              ...
                              className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-slate-700/60 bg-slate-800/50 text-cyan-400 hover:text-cyan-300 hover:bg-slate-800 transition-colors text-sm"
                              title="Download a sample script template"
                            >
                              <FileText className="w-4 h-4" />
                              Template
                            </button>
        ...
        {data.recent_runs.length > 0 && (
          <div
            className="rounded-2xl border border-slate-800/60 bg-slate-900/40 p-6 animate-fadeIn"
