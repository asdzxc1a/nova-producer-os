...
const GITIGNORE_COMMENT: &str = "# Nova local artifacts";
const GITIGNORE_ENTRIES: [&str; 3] = [".nova/settings.local.json", ".nova/sessions/", ".clawhip/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let claw_dir = cwd.join(".nova");
    artifacts.push(InitArtifact {
        name: ".nova/",
        status: ensure_dir(&claw_dir)?,
    });

    let claw_json = cwd.join(".nova.json");
    artifacts.push(InitArtifact {
        name: ".nova.json",
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    let mut lines = vec![
        "# CLAUDE.md".to_string(),
        String::new(),
        "This file provides guidance to Nova (clawcode.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.nova.json`; reserve `.nova/settings.local.json` for machine-local overrides.".to_string());
...
mod tests {
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".nova.json"));
        assert!(rendered.contains("created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("CLAUDE.md        created"));
        assert!(root.join(".nova").is_dir());
        assert!(root.join(".nova.json").is_file());
        assert!(root.join("CLAUDE.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".nova.json")).expect("read nova json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".nova/settings.local.json"));
        assert!(gitignore.contains(".nova/sessions/"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join(".gitignore"), ".nova/settings.local.json\n").expect("write gitignore");
        ...
        assert!(second_rendered.contains(".nova/"));
        assert!(second_rendered.contains(".nova.json"));
        ...
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".nova/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".nova/sessions/").count(), 1);
